...
class BaseAPIView_LatestNew(BaseAPI,CreateModelMixin):
    
    def get(self, request, *args, **kwargs):
        try:
            
            latest_id = request.GET.get('id',0)
            
            if 'pk' in kwargs:
                instance = self.get_object()
                self.serializer = self.get_serializer(instance)
                return Response(self.serializer.data)
            queryset = self.queryset.filter(pk__gt=latest_id)
...
class BaseListTagAPIView(BaseAPI,CreateModelMixin):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            queryset = self.filter_queryset(self.queryset).first().tags.similar_objects()
